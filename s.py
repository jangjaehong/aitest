...
question, answer = load_data(chat_log)
word_to_idx, idx_to_word = make_dic(contents=question + answer)
dic_len = len(word_to_idx)
print(dic_len)
print(word_to_idx)
...
def make_batch(seq_data):
    ...
    for seq in seq_data:
        ...
        input = [word_to_idx[n] for n in seq[0]]
        # 디코더 셀의 입력값. 시작을 나타내는 S 심볼을 맨 앞에 붙여준다.
        output = [word_to_idx[n] for n in (['<S>'] + seq[1])]
        # 학습을 위해 비교할 디코더 셀의 출력값. 끝나는 것을 알려주기 위해 마지막에 E 를 붙인다.
        target = [word_to_idx[n] for n in (seq[1] + ['<E>'])]
...
def translate(word):
    ...
    decoded = [idx_to_word[i] for i in result[0]]
